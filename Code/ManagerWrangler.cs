...
namespace ManagerWrangler
{
	...
	public class ManagerWrangler : MonoBehaviour
	{
		...
		Manager[] managers;
		List<CoroutineData> routines = new List<CoroutineData> ();
		...
		/// </summary>
		IEnumerator CoroutineUpdatePump ()
		{
			while (true) {
		...
		IEnumerator CoroutineFixedUpdatePump ()
		{
			while (true) {
				for (int i = 0; i < routines.Count; i++) {
		...
		object InvokeMember (object script, string methodName)
		{
		...
		/// </summary>
		object InvokeMember (object script, string methodName, params object[] parameters)
		{
		...
		/// </summary>
		MethodInfo GetCoroutineMethod (object script, string methodName)
		{
